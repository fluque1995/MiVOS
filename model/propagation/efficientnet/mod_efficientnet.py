...
from util.pytorch_privates import _log_api_usage_once, _make_divisible
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        layers.append(
            ConvNormActivation(
                3+extra_channels, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=nn.SiLU
            )
        )
...
def _efficientnet(
    ...
    **kwargs: Any,
    ...
    model = EfficientNet(inverted_residual_setting, dropout, extra_channels=extra_channels, **kwargs)
    if pretrained:
        if model_urls.get(arch, None) is None:
            raise ValueError(f"No checkpoint is available for model type {arch}")
        state_dict = load_state_dict_from_url(model_urls[arch], progress=progress)
        load_weights_sequential(model, state_dict, extra_channels)
    return model


def efficientnet_b0(extra_channels: int = 1, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> EfficientNet:
    """
    ...
    """
    return _efficientnet( "efficientnet_b0", 1.0, 1.0, 0.2, pretrained, progress, extra_channels, **kwargs)
