...
for patient in sorted(os.listdir(masks_dir)):
    for visit in sorted(os.listdir(os.path.join(masks_dir, patient))):
        for experiment in sorted(
                os.listdir(os.path.join(masks_dir, patient, visit))):
            ...
            if os.path.exists(masks_path):
                ...
                first_frame = cv2.cvtColor(cv2.imread(first_frame_path),
                                           cv2.COLOR_BGR2RGB)
                print(f'{patient}-{visit}-{experiment} loaded.')

                if experiment[0:5] == 'Suero':
                    extremes = extract_extreme_points(masks,
                                                      normalize=True,
                                                      move_to_origin=True)
                    ...
                    curr_points = extract_centers(masks,
                                                  normalize=True,
                                                  move_to_origin=True)
                ...
                print(
                    f'{patient}-{visit}-{experiment} masks not found. Skipping.'
                )
